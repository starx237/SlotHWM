...
from statm.modules import video
...
class SegmentationEncoderStateInit(nn.Module):
    ...
    @nn.compact
    def __call__(self, inputs: Array, batch_size: Optional[int],
                 train: bool = False) -> Array:
...
class NoHintsEncoderStateInit(nn.Module):
    """State init that encodes no conditional input."""
    slot_size: int = 128
    num_slots: int = 11

    @nn.compact
    def __call__(self, conditioning, batch_size, train=False):
        batch_size = batch_size
        key = self.make_rng("state_init")

        glorot_init = nn.initializers.xavier_uniform()
        slots_mu = self.param('slots_mu', glorot_init, (1, 1, self.slot_size))
        slots_log_sigma = self.param('slots_log_sigma', glorot_init, (1, 1, self.slot_size))

        slots = slots_mu + jnp.exp(slots_log_sigma) * random.normal(
            key, (batch_size, self.num_slots, self.slot_size))
