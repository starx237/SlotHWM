...
def standardize_loss_config(
        loss_config: Union[Sequence[str], ml_collections.ConfigDict]
  ...
  """
    ...
    # 形如loss_config = ml_collections.ConfigDict({
    #     "recon_flow": {
    #         "loss_type": "recon",
    #         "key": “flow”
    #         “weight”：1 # 这里的weight是可选的，可以不写，不写在compute_loss_term中会默认为1
    #     }
    # })
    loss_config = {
        k: {
...
def register_loss(func=None,
    ...
    # 注册损失函数，将其加入到_LOSS_FUNCTIONS字典中，以便后续调用，现在只加入了recon损失函数
...
def compute_loss_term(
    ...
    损失一个损失，例如光流或者深度的重构损失，并在compute_full_loss中求和
  Takes care of:
  ...
  Args:
    ...
    config_kwargs: The config dict for this loss. for example :{"loss_type": "recon","key": "flow"}
  Returns:
  ...
  """
    ...
    # 从_LOSS_FUNCTIONS字典中获取loss_name对应的损失函数
    # 如果config中没有loss_type，则默认为loss_name,即recon（重构）
