...
from statm.lib import preprocessing
...
def create_datasets(
  ...
  """
    ...
    val_batch_dims = get_batch_dims(config.val_batch_size)
    ...
    eval_split_name = config.get("validation_split", "validation") # iid
    # eval_split_name = config.get("validation_split", "test") # ood evaluation ,just used in testing

    train_split = deterministic_data.get_read_instruction_for_host(
        train_split_name, dataset_info=dataset_builder.info)
    # train_split = deterministic_data.split_for_jax_process(
    #     train_split_name, dataset_info=dataset_builder.info)
