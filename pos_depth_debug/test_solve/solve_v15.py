#!/usr/bin/env python3
"""
v15: 5步GD求depth + App Predictor求appearance
"""
import sys, os
...
def train_app_predictor(model, slots_c, alpha_out, fg_slots, bg_slots_list,
    ...
    for epoch in range(200):
        predictor.train()
        pred = predictor(frames_batch, train_pos.unsqueeze(1), train_depth.unsqueeze(1)).squeeze(1)
...
def main():
    ...
    for s_target in fg_slots:
        ...
        for t in range(T):
            ...
            # 5-step GD with trust region (|Δd| ≤ 0.008 per step)
            ...
            for _ in range(5):
                ...
                l = (sa - nofg_sharp_acovs[s_target][t]) ** 2
                opt_d.zero_grad(); l.backward(); opt_d.step()
                with torch.no_grad():
                    d_var.data.clamp_(0.01, 0.5)
                    d_var.data.clamp_(d_orig - 0.04, d_orig + 0.04)
            d_t = d_var.item()
    ...
    print(f"\nDepth solve time: {t_depth:.2f}s")

    # Step 2: Reference depths (20-step GD) for comparison
    print("\n=== Step 2: Reference depths (20-step GD) ===")
    ref_d = {}
    for s in fg_slots:
    ...
    print("\n=== Step 3: Train App Predictor ===")
    t0 = time.time()
    # 用 20-step ref_d 训练（更准确的目标）
    app_pred = train_app_predictor(model, slots_c, alpha_out, fg_slots, bg_slots_list,
                                   app_dim, frames, T, ref_d)
    t_app_train = time.time() - t0
    print(f"App Predictor training time: {t_app_train:.1f}s")

    # Step 4: Inference with App Predictor
    print("\n=== Step 4: App Predictor inference ===")
    t0 = time.time()
    for s_target in fg_slots:
        best_t = solved[s_target][0]['best_t']
        frame_list = [frames[0, t:t+1] for t in range(T)]
        frames_batch = torch.stack(frame_list)  # (T, 1, 3, 64, 64)
        ...
        # AppPredictor expects (B, T, 3, H, W), pos (B, T, 2), depth (B, T, 1)
        frames_input = frames_batch.squeeze(1).unsqueeze(0)  # (1, T, 3, 64, 64)
        with torch.no_grad():
            pred_app = app_pred(frames_input.cuda(),
                               pos_batch.unsqueeze(0).cuda(),
                               depth_batch.unsqueeze(0).cuda())
            pred_app = pred_app.squeeze(0)  # (T, app_dim)
    ...
    print(f"{'='*60}")
    print(f"Depth (5-step GD):     {t_depth:.2f}s")
    ...
    # Quality assessment: reconstruction MSE
    ...
    for s_target in fg_slots:
        ...
        for t in range(T):
            ...
            # Row 5: predicted a_t + solved d_t NoFG decode
        ...
        plt.suptitle(f'Slot {s_target}  [BestApp t={best_t}]', fontsize=12, fontweight='bold')
        plt.tight_layout()
        plt.savefig(f'{OUT}/v15_solved_slot{s_target}.png', dpi=130, bbox_inches='tight')
        plt.close()
        print(f"  Saved: v15_solved_slot{s_target}.png")
    ...
    for idx, s in enumerate(fg_slots):
        ...
        axes[0].plot(fr, d_refs, 'o', color=colors[idx], markersize=4, alpha=0.4, label=f'Slot {s} ref(20step)')
        axes[0].plot(fr, d_ts, 's-', markersize=3, color=colors[idx], label=f'Slot {s} 5step')
    ...
    axes[0].set_title('d_t: orig / ref(20step) / 5step')
    ...
    plt.suptitle('v15: 5step GD depth + AppPredictor', fontsize=13)
    plt.tight_layout()
    plt.savefig(f'{OUT}/v15_solved_metrics.png', dpi=150, bbox_inches='tight')
    plt.close()
    ...
    for s in fg_slots:
        ...
        for t in range(T):
            ...
            d_s = solved[s][t]['d_t']
            d_r = ref_d[s][t]
            print(f"  t={t:>2}: d_orig={d_o:.4f}, d_5step={d_s:.4f}, d_ref={d_r:.4f}, "
                  f"err_5step={abs(d_s-d_r):.4f}, err_orig={abs(d_o-d_r):.4f}")
