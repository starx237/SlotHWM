...
def eval_model(mdl, ds):
    mdl.eval()
    pos_l, depth_l, bnd_pos_l, bnd_depth_l = [], [], [], []
    for seed in [42, 99, 123, 456, 789]:
        ...
        pos_l.append(float(((pred[...,:2]-tgt[...,:2])**2*dm).sum()/(dm.sum()*2+1e-8)))
        depth_l.append(float(((pred[...,2:3]-tgt[...,2:3])**2*dm).sum()/(dm.sum()+1e-8)))
        bnd_pos_l.append(float(((pred[...,:2]-tgt[...,:2])**2*bnd).sum()/(bnd.sum()*2+1e-8)))
        bnd_depth_l.append(float(((pred[...,2:3]-tgt[...,2:3])**2*bnd).sum()/(bnd.sum()+1e-8)))
    return np.mean(pos_l), np.mean(depth_l), np.mean(bnd_pos_l), np.mean(bnd_depth_l)
...
# 评估
pos, depth, bnd_pos, bnd_depth = eval_model(m, ds)
print(f"\nTrained (pos1_d3, 800step): pos={pos:.6f} depth={depth:.6f} bnd_pos={bnd_pos:.6f} bnd_depth={bnd_depth:.6f}")

# 基线
base = make()
pos, depth, bnd_pos, bnd_depth = eval_model(base, ds)
print(f"Base:                        pos={pos:.6f} depth={depth:.6f} bnd_pos={bnd_pos:.6f} bnd_depth={bnd_depth:.6f}")
...
for t in range(rollout):
    b_t = bnd[:,t]
    bp = float(((pred[:,t,:,:2]-tgt[:,t,:,:2])**2*b_t).sum()/(b_t.sum()*2+1e-8))
    bd = float(((pred[:,t,:,2:3]-tgt[:,t,:,2:3])**2*b_t).sum()/(b_t.sum()+1e-8))
    print(f"  t={t}: bnd_pos={bp:.6f} bnd_depth={bd:.6f}")
